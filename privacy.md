...
- **You can delete everything**, from inside the app, without emailing anyone. There are two deliberate exceptions, both described in full below, and they share one reason: a record you could clear by signing up again would protect nobody. If an organizer removed you from a race, a minimal record of that removal outlives your account. And if someone reported something you posted, that report outlives both accounts: it keeps a quotation of up to 200 characters of the text that was reported — your own words, as you wrote them — together with an identifier for your account, and never the identity of whoever reported it.
...
- The changes you have kept to a plan. When you swap an exercise, add one, or change what a movement asks for and then finish the session, your phone keeps that change so the session comes back the way you left it. It is a short list of exercises and numbers, stored on your phone beside your programs, and the program it applies to is never rewritten. It goes nowhere by itself, with one exception you control: if you have switched on "share my workouts" for a coach, the changes standing on a session travel with that session's summary, which is described under "Sharing your workout history with a coach" below.
...
| Reports about a race photograph | When someone reports a photograph in a race: which photograph, who reported it, whose photograph it was, which of three broad categories they chose, when, and what the organizer decided. **Never any free text** — there is nowhere to type a description of a picture or a person. The organizer reviewing the report is never shown who made it. A report is deleted with the race it belongs to, or with the reporter's account |
| Reports about anything else | When someone reports something that is not a photograph — a shared plan or route, a workout, a report back, a friend request, or the name on a challenge, a group, a race or a person: what kind of thing it was, which one, who reported it, whose it was, which of the same three broad categories they chose, when, and what was decided. **Still nothing anybody typed**: the reporter chooses a category and there is no free-text box. Alongside it we keep **a copy of the offending text itself**, taken by our server from the content it was shown, cut off at 200 characters — without it, a complaint reviewed a day later can be about content that is already gone. It is never a photograph and never a link to one. **The person who reported is shown to nobody** — not to the person reported, and not to whoever reviews it, who sees how many different people complained and never which people. **These reports outlive both accounts**, for the reason given under "Being removed from a race": what they add up to is a count across accounts, and a record you could clear by signing up again would protect nobody |
| Your block list | Who you have blocked, and when. Yours alone: it is shown to nobody else, nothing anywhere tells another person they have been blocked or answers "who has blocked me", and it is deleted when either account is deleted |
...
| Suspension records | If we suspend an account: the Apple identifier, the account id and handle as they were at the time, a short internal note about why, who suspended it, and when. No content, and nothing about what was posted. **This one also survives account deletion**, for the reason given under "If an account is suspended" below |
...
| Workouts shared with a coach | Only if you turn on "share my workouts" for a specific friend: a summary of each workout you finish from then on — the exercises, the completed sets with their weights, times and distances, what you skipped, and, for an exercise you changed, the prescription your plan asked for beside the one it ended up at and whether you or your coach made the change. One copy per person you share with, capped at your most recent sixty sessions each, deleted when you turn that person off, when either of you removes or blocks the other, or when either account is deleted |
...
| Diagnostic mode | Off by default on the App Store, on by default in TestFlight builds; you can change it either way in Settings. Part of it runs always, whatever the setting: a small record of your last 5 launches, sign-ins, and App Store activity, including the product ids and store prices the app saw, your entitled tier, and a hashed transaction id, kept at most 200 KB and 7 days, so a launch that fails can still be diagnosed afterward. Turning Diagnostic mode on additionally records your workouts, GPS accuracy bands (never your coordinates), and network activity, kept up to 7 days and capped at about 4 MB. All of it stays on your phone. It reaches us only if you turn on "Attach detailed log" when you send feedback, or if you export it yourself from Settings |
...
**What is never on our server, unless you deliberately send it.** Your coach conversations, sleep, resting heart rate, HRV, body weight, injuries, goals, birth year, and home coordinate are never uploaded at all, under any setting. Your workouts, sets, weights and programs are not uploaded either — with the two exceptions named in the table above, both of which require you to act: a program you choose to send to a friend, and, if you switch on "share my workouts" for a specific person, a summary of the workouts you finish from then on. Nothing in this list leaves your phone because of a default.
...
Separately from share-back, you can give one specific friend a standing permission to see the workouts you finish — the arrangement a personal trainer and their client want. It follows the same rules as share-back with four differences worth stating plainly.

- **It is per person, off by default, and only ever a friend.** You turn it on for a named person, from the History tab or the privacy screen. Nothing anywhere turns it on for you, and there is no way to share with somebody you have not added as a friend.
- **Once on, it is automatic going forward — and only forward.** Each workout you finish from that point is sent to that person as a summary: the exercises, the sets you completed with their weights, times and distances, and what you skipped. Never your warm-ups, your reps in reserve, your notes, or anything from Apple Health or Fitbit. Workouts you finished before you turned it on are never sent, and our server refuses one that predates the permission even if an app were to send it.
- **A change you made to their plan goes with the workout it was made in.** This is the one thing in the summary that is not a record of what you did, and it is here because without it a coach sees dumbbell presses with no way to know their plan asked for a barbell. If you swapped an exercise, added one, or changed what a movement asks for, the summary carries the prescription the plan asked for, the prescription the exercise ended up at, and whether you or your coach made the change. Two sets of numbers and a name each: never the exercise's notes, its rest time, or its reps-in-reserve target, and never any free text. An exercise you did not change carries nothing of the kind. This is also the only part of the summary that describes your plan rather than your performance, and if you change nothing, nothing about your plan is sent.
- **Turning it off deletes everything they were shown.** So does removing them as a friend, or a block in either direction: the copies are removed from our server, not hidden. We keep at most your sixty most recent shared sessions per person; older ones are deleted as new ones arrive.
...
**Anyone who can see a photograph can report it.** Reporting offers three broad reasons — it doesn't belong in this race, it's inappropriate, it's harmful — and choosing one is the whole report; there is deliberately nowhere to type anything. The moment a report lands, the photograph is **hidden from everyone at once**, including the person who posted it, and it stays hidden until the race's organizer looks at it. The organizer sees the photograph and the reason and decides: put it back, or take it down for good. **The organizer is never told who reported it**, and neither is the person whose photograph it was. If you report something, the only sign anyone else could ever notice is the photograph being gone.

**A reported photograph's file is preserved, even if it would otherwise be deleted.** Every deletion clock above — the six months, the two-week review window, taking your own photograph down, being removed from a race — normally destroys the file. A report changes that: while a photograph has been reported, its file is kept even though nobody can see it anywhere in the app, because destroying the one copy of something somebody just asked us to look at is not defensible, and the law can require content connected to a report to be preserved. Held that way, it is visible to no one — not the race, not the organizer's gallery, not the person who posted it — and it is never used for anything except answering that report if it has to be answered. This is the single exception to every photograph-deletion promise on this page.

**You can block another person.** Blocking is done from their photograph in a race, and undone in **Settings > Blocked people**. It works in both directions and it reaches everywhere: you stop seeing their photographs, cheers, shared programs, shared workouts and routes, and any friend request from them, and they stop seeing yours — and any standing workout-history sharing between you ends, with what was already shared deleted. If you were friends here, blocking ends the friendship, and unblocking does not restore it — you would both have to add each other again. On competitive boards — a race roster, a challenge's standings, a league table, a group's member list — the blocked person's row is not deleted, because removing it would make the ranks and counts you see untrue; instead it appears as a nameless "Hidden participant" with a neutral icon, its scores and arrivals intact. **Nobody is told they have been blocked** and nothing anywhere answers "who has blocked me". What we store is the list itself: who you blocked and when, deleted when either account is deleted.
...
**We can suspend an account from WorkoutSmith.** This is separate from anything an organizer can do: an organizer decides who is in their races, and only we can decide that somebody is out of the app. It is what happens when a report about objectionable content or abuse turns out to be right, and it is the reason the community rules you accept before you train say what they say.

**What a suspension does.** The account cannot sign in, and any device already signed in stops working immediately rather than at the end of some window. Photographs that account added to races are taken down: they stop being shown in the race, to other people in it, and to the account that posted them. The one place a photograph stays visible is to the organizer of the race it was in, who is the person responsible for that race and can delete it outright.

**What a suspension does not do.** It does not take the account's name off a race roster or a leaderboard. Those rows are a record of a competition other people were also in, and blanking one would both misstate what happened and announce to everybody in that race that a specific named person had been suspended — which is not ours to broadcast.

**We keep a record of the suspension, and it deliberately outlives account deletion.** It is the identifier Apple gave us for the account, its id and handle as they stood, a short note we write for ourselves about why, who made the decision, and when. **That is the whole record**, and it holds nothing about what was posted. It outlives deletion for exactly the reason the removal record above does: if deleting an account cleared it, deleting and signing up again would undo every suspension, and the whole thing would be a gesture.

**A suspension can be lifted.** There is no separate permanent state — a suspension we never lift is simply one we never lift. If you think yours is a mistake, the feedback form in the app still works and still reaches us with your account attached, which is on purpose: it is the one channel a suspended account keeps, and it is how you tell us we got it wrong. You can also write to the address at the bottom of this page.
...
WorkoutSmith sells two monthly subscriptions inside the app: **WorkoutSmith Premium** and **WorkoutSmith Coach**. Both are auto-renewing, both are monthly, and neither is shareable with a family group. The only functional difference between them is how many people you may coach at once: Premium lets you coach up to two people concurrently, Coach lifts that limit. A subscription unlocks starting a workout, building a program, and the competitive surfaces. Your history, your progress, the AI coach and the voice trainer, Adventure, and everything in Settings including data export and account deletion stay open whether or not you are subscribed.

**Apple takes the payment, not us. We never see a card.** The purchase runs entirely through Apple's in-app purchase system. There is no card field anywhere in this app, no payment processor of any kind is integrated into it, and no billing address, card number, or payment credential is ever entered into, held by, or transmitted by WorkoutSmith. Payment is charged to your Apple ID, and you manage or cancel the subscription in your Apple account settings rather than here. What Apple does with the transaction is governed by Apple's own privacy policy, not this one.

**Our server is not told that you subscribed.** This is the part worth being precise about, because it is unusual. There is no endpoint on our server that accepts a receipt, a transaction, or an entitlement, and there is no field on any record we hold that says whether you pay us. Whether you are entitled is read on your own device from Apple's transaction store, which is why the app can answer the question with the network off. None of that reaches us in the course of normal use. The one way it can is if you turn on Diagnostic mode and then choose to attach the detailed log to a feedback report yourself; what that carries is described in "What we store on our server" above.

**We do not keep your subscription state in a database, and it stays out of the data export, the home-screen widget, and everything sent to the AI coach.** The status the app actually gates on is held in memory while the app runs and re-read from Apple each time it launches; that live value touches no database or keychain of ours. Separately, the always-on diagnostic core (see "Diagnostic mode" above) keeps a small file on your phone that includes a record of your entitled tier and recent App Store activity, so a launch that goes wrong can be diagnosed afterward. That file stays on your phone unless you attach it to feedback or export it yourself.

**The free period is Apple's to grant and Apple's to judge.** New subscribers may be offered an introductory free period. Whether you are eligible is decided by Apple against your Apple ID across the whole subscription group, not by us and not by anything we store, and taking the free period on one tier does not earn a second one on the other. We are told only whether the entitlement currently in force came from an introductory offer, so the app can show how many days are left. We do not store that in our database. It can end up in the on-device diagnostic file described under "Diagnostic mode" above, which stays on your phone unless you send it to us yourself.

**There are two ways a subscription detail can reach us, and only the first one is still a single word.** If you send feedback and tick the box that attaches the basic diagnostic log, that log still contains only one line about your subscription: whether the app considered you `unknown`, `entitled`, `trial`, or `locked` at the time. That box is off by default. Separately, if you have turned on Diagnostic mode in Settings and you tick "Attach detailed log" when you send feedback, that log carries more: the product ids the app saw, their store-displayed prices, your entitled tier (so it does distinguish Premium from Coach), and a hashed transaction id. Both boxes are off by default, both logs are shown to you before you send them, and neither ever carries your Apple ID, your card details, or a receipt.

---

## Your programs and history in a Google spreadsheet

This is optional, off until you set it up, and it moves your training data to a file in **your own Google Drive, under your own Google account**. It is disclosed here in its own section because connecting a third-party account and sending training data out of the app is a data flow you should be able to read about in one place.

**You sign in to Google, not to us.** Consent happens on Google's own page in a private browser session, and we never see or hold your Google password. We hold no account of yours with Google and pay for no storage: the spreadsheet is created in your Drive, on your quota, and it is your file.

**Two permissions, and it is worth knowing exactly what the wider one means.** The app asks for permission to work with Google Sheets, and for permission covering only files it created or that you handed to it. The second is narrow and leaves the rest of your Drive invisible to the app. **The first is not narrow**, and we would rather say so than let you assume otherwise: Google classifies it as sensitive, and it covers reading and writing a spreadsheet you point the app at, not only spreadsheets the app made. You can grant one permission without the other, and the app checks what you actually granted rather than assuming both.

**What goes out.** A history sheet carries one row per set you logged: the date, the program, week and session, the exercise and how it was prescribed, the set number, whether it was a warm-up, target and actual reps, weight, duration, distance, reps in reserve, and whether the set was completed. A program sheet carries the plan only: day, exercise, sets, reps, duration, distance, rest, and the target heart-rate zone the plan asks for. **Neither sheet carries your body weight, your height, your birth year, your home location, anything from Apple Health or Fitbit, any measured heart rate, any note you typed, or anything from your conversations with the coach.** The heart-rate zone on a program sheet is a planned target, not a reading from you.

**What comes back, and how little we trust it.** A program can be imported from a spreadsheet. **Results never can.** A history sheet is refused outright at the first cell, deliberately, because a number typed into a cell is not evidence that you lifted anything. Everything that does come in from a file is treated as text somebody else may have written: every imported cell is stripped of the characters used to smuggle instructions, exercise names must match the app's own catalog or the row is refused, every number is range-checked, and a column the app does not recognise is refused rather than quietly ignored. If a program that came out of a file is later described to the AI coach, its text is fenced as untrusted first. The app records that a program came from a file, and records no link, no filename, and no Google account alongside it.

**Our server is not part of this.** The sign-in and every call to Google happen on your device, directly between your phone and Google. Our server never receives the spreadsheet, its contents, its identifier, or your Google token. (The private triage spreadsheet described under "Feedback you send us" is a different thing entirely: it is the developer's own file, on the developer's own account, and it has nothing to do with your Drive.)

**We learn nothing about your Google identity.** The permissions above cannot return one. We hold no Google email address, no name, no Google account id, no contacts, and no calendar.

**Where the key is kept, and what disconnecting does.** The access Google gives us is stored in your iPhone's keychain, marked so that it does not sync to iCloud and is not carried in an encrypted backup. Disconnecting in Settings erases it from the phone and asks Google to revoke it. The erase always happens; the revoke is a network call, so if it fails you may still see the grant listed in your Google account until you remove it there. Erasing all data on the device clears it too. **Disconnecting does not delete spreadsheets you already exported.** They are your files, in your Drive, and they stay where they are until you delete them.
...
Your programs and your workout history can also go to a Google spreadsheet in your own Google Drive, under your own Google account, and a program can come back in from one. That is an optional integration with a third party and it is described in full under "Your programs and history in a Google spreadsheet" above, including which permissions the app asks Google for and what each sheet does and does not carry.
...
Five things survive by design, and you should know before you press it:
...
- **A report somebody made about something you posted stays, and it stays identified — including a quotation of what you wrote.** If another person reported a program, route, challenge, group, race, race stop or name of yours, that report survives your account. It holds the category they chose, an identifier for your account that outlives the account itself, and **a verbatim excerpt of up to 200 characters of the reported text**. For almost every kind of report that excerpt is the name you gave something; for a reported stop on a race route it can include the note you wrote about it. It never holds who reported you. It survives for the same reason the race-removal record does: a complaint that the person complained about could delete is not a moderation system.
- **A record that we suspended your account stays, and it stays identified.** The Apple identifier, the account id and handle as they were at the time, a short internal note about why, who suspended it, and when. No content and nothing about what was posted. It survives because a ban you could clear by deleting your account and signing up again would not be a ban.
...
- **There is a rule about behaviour and it is enforced.** Everyone accepts community rules in the app before they train: no objectionable content and no abuse of other people, with zero tolerance for either. Anyone can report a photograph they can see, which hides it from everyone until the race's organizer decides about it, and anyone can block another person, which removes the two of you from each other's view throughout the app — photographs and cheers disappear, and on competitive boards each appears to the other as a nameless hidden participant. An organizer can remove anyone from their race, removals are recorded, and an account that different organizers have repeatedly removed can be banned from the app. **We can also suspend an account outright when a report turns out to be right**, which stops it signing in on any device immediately and takes its race photographs down; a suspension record survives account deletion, so deleting and signing up again does not clear it. A reported photograph's file is preserved while the report needs it, hidden from everyone — the one exception to the photograph-deletion promises above.
...
### The subscription
- Two monthly products and the only difference between them: `ios/AIWorkout/Core/Subscription/SubscriptionManager.swift` (`SubscriptionTier`, `SubscriptionProduct.allIDs`), `ios/AIWorkout/Core/Subscription/CoachingAllowance.swift` (`baseTierLimit`, `verdict(status:currentTraineeCount:)`)
- What a subscription gates and what stays open regardless: `ios/AIWorkout/Core/Subscription/EntitlementStatus.swift` (`GatedSurface`, `GatedSurface.openWhenLocked`), applied by `ios/AIWorkout/Features/Paywall/PaywallGate.swift`
- Purchase and restore are Apple's own, with no payment processor in the project: `SubscriptionManager.swift` (`product.purchase()`, `AppStore.sync()`, `Transaction.currentEntitlements`, `Transaction.updates`); no card entry exists in `ios/AIWorkout/Features/Paywall/PaywallView.swift`, whose only actions are subscribe, restore and dismiss
- Nothing about a purchase reaches the server: no receipt, entitlement, transaction or App Store Server Notification route exists in `server/src/app.ts`, and no entitlement field exists in `server/src/store/types.ts` or `server/src/db.ts`. Apple appears server-side only for Sign in with Apple, in `server/src/apple.ts`
- Entitlement is in memory only, re-read from Apple at launch: `EntitlementStatus` is a plain struct with no persistence, and no entitlement key is written to SwiftData, UserDefaults or the keychain. Absent from `ios/AIWorkout/Features/Settings/WorkoutDataExport.swift`, from `ios/AIWorkoutWidgets/`, and from `ios/AIWorkout/Core/TrainerContextBuilder.swift`
- The introductory period is read off the live product and never hardcoded: `SubscriptionManager.swift` (`IntroductoryOffer`, `refreshIntroductoryOffer()`, `isEligibleForIntroOffer`); days remaining rendered only at `EntitlementStatus.settingsDetail()`
- The single word a diagnostic log can carry, with no tier in it: `ios/ProgressionEngine/Sources/DiagnosticLog/DiagEvent.swift` (`DiagEntitlementState`), rendered in `DiagTranscript.swift`; attachment is off by default at `ios/AIWorkout/Features/Feedback/FeedbackFormView.swift`
- No purchase or payment data type is declared in `PrivacyInfo.xcprivacy` for any of the three targets

### Google Sheets
- The two scopes, and the note that one of them is Google-classified sensitive and is not limited to app-created files: `ios/AIWorkout/Core/Integrations/GoogleSheets/GoogleSheetsConfig.swift` (`spreadsheetsScope`, `driveFileScope`, `scopes`); the only place they are sent is `GoogleSheetsOAuth.swift` (`authorizationURL`). Either may be granted alone, and the app reads what was granted: `GoogleSheetsManager.swift` (`grantsSpreadsheets`, `grantsFileCreation`), `SheetsGate.swift`
- A separate OAuth client and a separate keychain item from the Google Health grant: `GoogleSheetsConfig.swift`, `ios/AIWorkout/Core/AppConfig.swift`
